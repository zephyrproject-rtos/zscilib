...
ZTEST(zsl_tests, test_phy_photon_mom)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(p, 0.0007519396, 1E-6));
...
ZTEST(zsl_tests, test_phy_photon_wavelenght)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(lambda, 0.5450771964, 1E-6));
...
ZTEST(zsl_tests, test_phy_photon_frequency)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, 5450.7719636363, 1E-6));
...
ZTEST(zsl_tests, test_phy_photon_photoelectric)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(kmax, 0.9746172333, 1E-6));

	/* Example for negative frequency. */
	rc = zsl_phy_photon_photoelectric(-5.5E14, 1.3, &kmax);
	zassert_true(rc == -EINVAL);
	/* IEEE standard states that x != x is true only for NAN values. */
	zassert_true(kmax != kmax);
