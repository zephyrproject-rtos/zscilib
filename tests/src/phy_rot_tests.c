...
ZTEST(zsl_tests, test_phy_rot_dist)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(dist, 6.6, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_turn)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(turn, 0.1750704374, 1E-6));
}
...
ZTEST(zsl_tests, test_phy_rot_time)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(time, 1.3, 1E-6));

	/* Example for negative angular acceleration. */
	rc = zsl_phy_rot_time(4.1, 3.1, -2.0, &time);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(time, 0.5, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_omega)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(of, 15.5, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_omega2)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(of, 5.8309518948, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_av_omega)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(omega, 0.5, 1E-6));

	/* Example for zero time. */
	rc = zsl_phy_rot_av_omega(1.0, 0.0, &omega);
	zassert_true(rc == -EINVAL);
	/* IEEE standard states that x != x is true only for NAN values. */
	zassert_true(omega != omega);
...
ZTEST(zsl_tests, test_phy_rot_vel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(vel, 19.2, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_ang_accel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(a, 0.25, 1E-6));

	/* Example for negative angular acceleration. */
	rc = zsl_phy_rot_ang_accel(4.5, 2.1, 1.2, &a);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(a, -2.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_accel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(accel, 9.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_ener)
{
	...
	zassert_true(rc == 0);
 	...
 	zassert_true(val_is_equal(rke, 78.802, 1E-5));
#else
        zassert_true(val_is_equal(rke, 78.802, 1E-8));
#endif

	/* Example for zero moment of inertia. */
	rc = zsl_phy_rot_ener(6.2, 0.0, &rke);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(rke, 0.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_period)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(t, 1.0134169850, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_frequency)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, 0.9867606472, 1E-6));

	/* Example for zero angular velocity. */
	rc = zsl_phy_rot_frequency(0.0, &f);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, 0.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_cent_accel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(ca, 8.8595238095, 1E-6));

	/* Example for zero tangencial velocity. */
	rc = zsl_phy_rot_cent_accel(0.0, 4.2, &ca);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(ca, 0.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_cent_accel2)
{
	...
	zassert_true(rc == 0);
#ifdef CONFIG_ZSL_SINGLE_PRECISION
 	zassert_true(val_is_equal(ca, 115.1453846794, 1E-5));
#else
        zassert_true(val_is_equal(ca, 115.1453846794, 1E-8));
#endif
...
ZTEST(zsl_tests, test_phy_rot_total_accel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(at, 6.6850579653, 1E-6));
...
ZTEST(zsl_tests, test_phy_rot_power)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(power, 34.17, 1E-6));
}
