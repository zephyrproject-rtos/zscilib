...
ZTEST(zsl_tests, test_phy_magn_force)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, 287.4143736198, 1E-6));
...
ZTEST(zsl_tests, test_phy_magn_force_wire)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, 5.7482874724, 1E-6));
...
ZTEST(zsl_tests, test_phy_magn_torque_loop)
{
	...
	zassert_true(val_is_equal(tor, 57.482874724, 1E-5));
#else
	zassert_true(val_is_equal(tor, 57.482874724, 1E-8));
#endif
...
ZTEST(zsl_tests, test_phy_magn_pot_dipole)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(u, -41.5448687614, 1E-6));
...
ZTEST(zsl_tests, test_phy_magn_orb_radius)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(r, 12.1625, 1E-6));
...
ZTEST(zsl_tests, test_phy_magn_flux)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(fl, 19.9922490518, 1E-6));
...
ZTEST(zsl_tests, test_phy_magn_mom)
{
	...
	zassert_true(rc == 0);
#ifdef CONFIG_ZSL_SINGLE_PRECISION
	zassert_true(val_is_equal(mom, 17.425, 1E-5));
#else
	zassert_true(val_is_equal(mom, 17.425, 1E-8));
#endif
