...
ZTEST(zsl_tests, test_phy_grav_orb_period)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(t, 8.598826195, 1E-6));
...
ZTEST(zsl_tests, test_phy_grav_esc_vel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(v, 18.2675668878, 1E-6));
...
ZTEST(zsl_tests, test_phy_grav_acc)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(a, -1.66852, 1E-6));
...
ZTEST(zsl_tests, test_phy_grav_orb_vel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(v, 12.9171204221, 1E-6));
...
ZTEST(zsl_tests, test_phy_grav_force)
{
	...
	zassert_true(val_is_equal(f, -53.39264, 1E-5));
#else
	zassert_true(val_is_equal(f, -53.39264, 1E-6));
#endif
...
ZTEST(zsl_tests, test_phy_grav_pot_ener)
{
	...
	zassert_true(rc == 0);
#ifdef CONFIG_ZSL_SINGLE_PRECISION
	zassert_true(val_is_equal(u, -26.69632, 1E-5));
#else
	zassert_true(val_is_equal(u, -26.69632, 1E-6));
#endif
