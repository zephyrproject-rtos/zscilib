...
{
        int rc;
        struct interp_xy xy1;
        struct interp_xy xy3;
        zsl_real_t y2;
...
void test_interp_lin_y(void)
{
    ...
    /* Test 1: Interpolate y at x = 13.6 */
    rc = interp_lin_y(&xy1, &xy3, 13.6f, &y2);
    zassert_equal(rc, 0, NULL);
    zassert_true(val_is_equal(y2, 0.2072f, 1E-4F), NULL);

    /* Test 2: Check for x too low. */
    rc = interp_lin_y(&xy1, &xy3, xy1.x - 0.01f, &y2);
    zassert_equal(rc, -EINVAL, NULL);

    /* Test 3: Check for x too high. */
    rc = interp_lin_y(&xy1, &xy3, xy3.x + 0.01f, &y2);
    zassert_equal(rc, -EINVAL, NULL);

    /* Test 4: Check for delta 0.0 on x. */
    xy3.x = xy1.x;
    rc = interp_lin_y(&xy1, &xy3, xy1.x, &y2);
    zassert_equal(rc, -EINVAL, NULL);
    xy3.x = 20.0f;
    ...
    xy1.x = -4.5f;
    xy3.x = 10.0f;
    rc = interp_lin_y(&xy1, &xy3, 2.75f, &y2);
    zassert_equal(rc, 0, NULL);
    ...
    /* Test 7: x2 == x3. */
    rc = interp_lin_y(&xy1, &xy3, xy3.x, &y2);
    zassert_equal(rc, 0, NULL);
    zassert_true(val_is_equal(y2, xy3.y, 1E-4F), NULL);
}
...
void test_interp_lin_x(void)
{
    ...
    rc = interp_lin_x(&xy1, &xy3, 0.2072f, &x2);
    zassert_equal(rc, 0, NULL);
    zassert_true(val_is_equal(x2, 13.6f, 1E-4F), NULL);

    /* Test 2: Check for y too low. */
    rc = interp_lin_y(&xy1, &xy3, xy1.y - 0.01f, &x2);
    zassert_equal(rc, -EINVAL, NULL);

    /* Test 3: Check for y too high. */
    rc = interp_lin_y(&xy1, &xy3, xy3.y + 0.01f, &x2);
    zassert_equal(rc, -EINVAL, NULL);

    /* Test 4: Check for delta 0.0 on x. */
    xy3.x = xy1.x;
    rc = interp_lin_x(&xy1, &xy3, xy1.y, &x2);
    zassert_equal(rc, -EINVAL, NULL);
    xy3.x = 20.0f;
    ...
    xy1.y = -0.20f;
    xy3.y = 0.20f;
    rc = interp_lin_x(&xy1, &xy3, -0.1f, &x2);
    zassert_equal(rc, 0, NULL);
    ...
    /* Test 7: y2 == y3. */
    rc = interp_lin_x(&xy1, &xy3, xy3.y, &x2);
    zassert_equal(rc, 0, NULL);
