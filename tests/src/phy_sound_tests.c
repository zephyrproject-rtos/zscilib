...
ZTEST(zsl_tests, test_phy_sound_press_amp)
{
	...
	zassert_true(val_is_equal(p, 6688.65, 1E-3));
#else
	zassert_true(val_is_equal(p, 6688.65, 1E-8));
#endif
...
ZTEST(zsl_tests, test_phy_sound_level)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(b, 27.2529894306, 1E-6));
...
ZTEST(zsl_tests, test_phy_sound_intensity)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(i, 21.0464001945, 1E-6));
...
ZTEST(zsl_tests, test_phy_sound_shock_wave_angle)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(theta, 0.6413745117, 1E-6));
...
ZTEST(zsl_tests, test_phy_sound_dop_effect)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(fl, 854.85, 1E-6));
...
ZTEST(zsl_tests, test_phy_sound_beat)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, 22.5, 1E-6));

	/* Example for 'fa' < 'fb'. */
	rc = zsl_phy_sound_beat(563.8, 697.9, &f);
	zassert_true(rc == 0);
#ifdef CONFIG_ZSL_SINGLE_PRECISION
	zassert_true(val_is_equal(f, 134.1, 1E-4));
#else
	zassert_true(val_is_equal(f, 134.1, 1E-8));
#endif
