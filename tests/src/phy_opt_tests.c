...
ZTEST(zsl_tests, test_phy_opt_refrac_index)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(n, 0.9, 1E-6));
...
ZTEST(zsl_tests, test_phy_opt_snell)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(o2, 0.5680829219, 1E-6));
...
ZTEST(zsl_tests, test_phy_opt_focus)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(f, -6.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_opt_critic_angle)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(oc, 0.6082455789, 1E-6));
...
ZTEST(zsl_tests, test_phy_opt_power)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(p, 0.5, 1E-6));

	/* Example for negative focal length. */
	rc = zsl_phy_opt_power(-5.0, &p);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(p, -0.2, 1E-6));
...
ZTEST(zsl_tests, test_phy_opt_dif)
{
	...
	zassert_true(rc == 0);
 	...
 	zassert_true(val_is_equal(d, 2106.4002522072, 1E-3));
#else
	zassert_true(val_is_equal(d, 2106.4002522072, 1E-8));
#endif
