...
ZTEST(zsl_tests, test_phy_mom_mom)
{
	...
	zassert_true(rc == 0);
 	zassert_true(val_is_equal(p, 79.65, 1E-6));
...
ZTEST(zsl_tests, test_phy_mom_imp)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(imp, 150.0, 1E-6));

	/* Example for negative time. */
	rc = zsl_phy_mom_imp(50.0, -3.0, &imp);
	zassert_true(rc == -EINVAL);
	/* IEEE standard states that x != x is true only for NAN values. */
	zassert_true(imp != imp);
}
...
ZTEST(zsl_tests, test_phy_mom_change)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(p, -159.6, 1E-6));
...
ZTEST(zsl_tests, test_phy_mom_elas)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(vf2, 2.7810810811, 1E-6));

	/* Example for negative mass 'm1'. */
	rc = zsl_phy_mom_elas(-13.1, 15.6, 9.6, 11.1, -4.3, &vf2);
	zassert_true(rc == -EINVAL);
	/* IEEE standard states that x != x is true only for NAN values. */
	zassert_true(vf2 != vf2);
...
ZTEST(zsl_tests, test_phy_mom_inelas)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(vf, 6.4723140496, 1E-6));
