...
ZTEST(zsl_tests, test_phy_proj_init_vel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(vi_hor, 8.660254038, 1E-6));
	zassert_true(val_is_equal(vi_ver, 5.0, 1E-6));
}
...
ZTEST(zsl_tests, test_phy_proj_time)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(t, 2.460370769, 1E-6));
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(t, 1.323253572, 1E-6));
...
ZTEST(zsl_tests, test_phy_proj_time_first)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(t, 0.3082341412, 1E-6));
...
ZTEST(zsl_tests, test_phy_proj_time2)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(t, 5.098399103, 1E-6));
...
ZTEST(zsl_tests, test_phy_proj_ver_motion)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(yf, 25.386, 1E-6));
...
ZTEST(zsl_tests, test_phy_proj_ver_vel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(vfv, 0.386, 1E-6));

	rc = zsl_phy_proj_ver_vel(20.0, -2.0, &vfv);
	zassert_true(rc == -EINVAL);
	/* IEEE standard states that x != x is true only for NAN values. */
	zassert_true(vfv != vfv);
}
...
ZTEST(zsl_tests, test_phy_proj_hor_motion)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(xf, 20.0, 1E-6));
...
ZTEST(zsl_tests, test_phy_proj_trajectory)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(yf, 11.0965, 1E-6));
...
ZTEST(zsl_tests, test_phy_proj_vel)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(vf, 5.0, 1E-6));
}
...
ZTEST(zsl_tests, test_phy_proj_angle)
{
	...
	zassert_true(rc == 0);
	zassert_true(val_is_equal(theta, 0.927295218, 1E-6));

	/* Example where the horizontal velocity is zero. */
	rc = zsl_phy_proj_angle(0.0, 4.0, &theta);
	zassert_true(rc == 0);
	zassert_true(val_is_equal(theta, (ZSL_PI / 2), 1E-6));
}
...
ZTEST(zsl_tests, test_phy_proj_range)
{
	...
	zassert_true(rc == 0);
 	...
 	zassert_true(val_is_equal(dist, 23.50711351, 1E-5));
#else
	zassert_true(val_is_equal(dist, 23.50711351, 1E-8));
#endif
}
